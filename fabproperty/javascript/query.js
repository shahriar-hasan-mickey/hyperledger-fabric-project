...
async function main( queryData ) {
    try {
        ...
        /// IF QUERY DATA WITH ID IS AVAILABLE
        if( queryData.key ){
            // console.log("key");
         ...
         /// IF QUERY DATA WITH CITY IS AVAILABLE
         else if( queryData.city ){
            // console.log("city")
            const queryResult =  await contract.evaluateTransaction('queryPropertyByCity', `${ queryData.city }`);
            console.log(`QUERY Transaction has been evaluated, result is: ${queryResult.toString()}`)
        ...
        // queryAllPropertys transaction - requires no arguments, ex: ('queryAllProperties')
